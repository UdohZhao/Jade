...
# 微信用户表「主键id，关联客服用户表主键id，openid，nickname，sex，province，city，country，headimgurl，privilege，unionid」
...
# 用户信息表{id，关联用户表主键id，账户余额}
CREATE TABLE `userinfo`(
  `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '用户信息表主键id',
  `wuid` int(11) UNSIGNED NOT NULL COMMENT '关联微信用户表主键id',
...
# 阅读消费表{id，关联微信用户表主键id，关联文章表主键id，消费金额，时间，类型（付费阅读，工作号免费阅读）}
...
# 商品评价表{id，关联微信用户表主键id，关联商品表主键id，商品规格，内容，时间，类型（好评，中评，差评）}
...
# 购物车表{id，关联微信用户表主键id，关联商品表主键id，规格，数量，时间}
...
# 收货地址表{id，关联微信用户表主键id，收货人，联系电话，收货地址，状态（正常，默认）}
...
# 订单表{id，关联微信用户表主键id，商品封面路径（序列化数组），商品名称（序列化数组），商品规格（序列化数组），购买数量（序列化数组），商品单价（序列化数组），合计价格，订单编号，收货信息，买家留言，下单时间，发货时间，操作时间，类型（0>待付款，1>待发货，2>待收货，3>待评价，4>售后），状态（0>正常，1>取消订单，2>订单超时，3>申请退款，4>退款失败，5>退款成功）}
CREATE TABLE `indent`(
...
# 充值记录表{id，关联微信用户表主键id，金额，时间，类型（正常充值，工作号优惠充值）}
